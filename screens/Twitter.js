...
export default function Twitter({ dispatch, user }) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (data.success) {
        ...
        const isTwitterLinked = socialsArray.some(
          (social) => social.platform_name === "twitter"
        );

        if (isTwitterLinked) {
          notifyMessage("Twitter account already linked");
        ...
        const secondResponse = await fetchWithTimeout(
          `https://lion-optimal-sawfish.ngrok-free.app/api/twitter/add_account`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
        ...
        if (secondResponse.ok) {
          ...
          if (secondData.success) {
            notifyMessage("Twitter account added successfully");
            dispatch({ type: "profileScreen", payload: true });
            handleLoader();
          } else {
            notifyMessage("Twitter account could not be added");
          }
        } else {
          setErr("Failed to add Twitter account. Please try again later.");
        }
      } else {
